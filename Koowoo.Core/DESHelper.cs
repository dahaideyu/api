...
namespace Koowoo.Core
{
    public class DESHelper
    {
        ...
        public static string Encrypt3Des(string aStrString, byte[] aStrKey, CipherMode mode = CipherMode.ECB, string iv = "12345678")
        {
            try
            {
                var des = new TripleDESCryptoServiceProvider
                {
                    Key = aStrKey , // Encoding.UTF8.GetBytes(aStrKey),
                    Mode = mode
                };
        ...
        public static string Decrypt3Des(string aStrString, byte[] aStrKey, CipherMode mode = CipherMode.ECB, string iv = "12345678")
        {
            try
            {
                var des = new TripleDESCryptoServiceProvider
